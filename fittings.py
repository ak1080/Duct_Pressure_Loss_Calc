...
class ASHRAE_CR3_1_SmoothRadiusElbow(Fitting):
    ...
    # Angle factor K
    K_TABLE = {
        20: 0.31, 30: 0.45, 45: 0.60, 60: 0.78,
...
class ASHRAE_ER5_2_Tee(Fitting):
    ...
    def from_user_input(cls):
        ...
        while True:
            try:
                main_cfm = int(input("Main duct CFM (Qs): "))
                if main_cfm <= 0:
        ...
        while True:
            try:
                branch_cfm = int(input("Branch duct CFM (Qb): "))
                if branch_cfm <= 0:
                    print("CFM must be positive.")
                    continue
                break
...
class ASHRAE_ER5_3_Tee(Fitting):
    ...
    def from_user_input(cls):
        ...
        main_cfm = positive_int("Main duct CFM (Qs): ")
        ...
        branch_cfm = positive_int("Branch duct CFM (Qb): ")
...
class SMACNA_Rectangular_Z_Elbow(Fitting):
    ...
    def _interpolate(x, table):
        ...
        if x <= keys[0]:
            return table[keys[0]]
        if x >= keys[-1]:
            #clamps to last available table value
            return table[keys[-1]]
    ...
    def loss_coefficient(self):
        ...
        if LH > max(self.LH_TABLE):
            #clamps to the max table value (L/H = 10, C = 2.5)
...
class SMACNA_Converging_Tee_Round:
    ...
    def from_user_input(cls):
        ...
        while True:
            try:
                Qs = int(input("Main duct airflow Qs (CFM): "))
                if Qs <= 0:
                    raise ValueError
                break
            except ValueError:
        ...
        while True:
            try:
                Qb = int(input("Branch duct airflow Qb (CFM): "))
                if Qb <= 0:
                    raise ValueError
                break
            except ValueError:
...
class SMACNA_Conical_Tee_Round:
    ...
    def __init__(self, main_diam_in, branch_diam_in, Qc, Qb, flow_type):
        self.main_diam_in = main_diam_in
        self.branch_diam_in = branch_diam_in
        self.Qc = Qc                  # upstream flow (user input)
        self.Qb = Qb                  # branch flow
        self.Qs = Qc - Qb             # straight-through downstream flow
        self.flow_type = flow_type    # "main" or "branch"
    ...
    def from_user_input(cls):
        ...
        while True:
            try:
                Qc = int(input("Upstream airflow Qc (CFM): "))
                if Qc <= 0:
                    raise ValueError
                break
            except ValueError:
        ...
        while True:
            try:
                Qb = int(input("Branch airflow Qb (CFM): "))
                if Qb <= 0 or Qb >= Qc:
                    raise ValueError
                break
            except ValueError:
    ...
    def results(self):
        ...
        # Velocities
        Vc = self.Qc / area_main        # Upstream velocity (used for VP)
        Vs = self.Qs / area_main        # Downstream straight-through
        Vb = self.Qb / area_branch      # Branch velocity

        # Upstream velocity pressure
        vp_c = (Vc / 4005) ** 2

        # Velocity to report in summary = upstream main
        velocity = Vc
        ...
        return {
            "flow_path": flow_path,
            "velocity": velocity,  # upstream main
            "vp": vp_c,
            "C": C_used,
            "loss": loss,
        }
    ...
    @property
    def cfm(self):
        # Always report upstream main airflow
        return self.Qc

    def dimensions(self):
        # Report upstream main airflow for main, branch airflow for branch
        return {
            "main": {
                "shape": "round",
                "diameter_in": self.main_diam_in,
                "cfm": self.Qc,  # <-- upstream main
            },
            "branch": {
                "shape": "round",
                "diameter_in": self.branch_diam_in,
...
class SMACNA_Rectangular_Main_Conical_Branch:
    ...
    def results(self):
        ...
        Vc = self.Qc / area_main  # upstream velocity
        Vs = self.Qs / area_main  # straight-through downstream
        Vb = self.Qb / area_branch

        vp_c = (Vc / 4005) ** 2
        velocity = Vc  # always report upstream main velocity
    ...
    @property
    def cfm(self):
        return self.Qc  # always report upstream main airflow

    def dimensions(self):
        # main reports Qc, branch reports Qb
        return {
            "main": {
                "shape": "rectangular",
                "width_in": self.main_width_in,
